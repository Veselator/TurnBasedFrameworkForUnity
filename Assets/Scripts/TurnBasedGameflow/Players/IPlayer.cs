public interface IPlayer
{
    // Интерфейс игрока для пошаговой системы
    string Name { get; }
    int ID { get; }

    // Эти две переменные нужны для гибкости подсчёта очков
    float Points { get; set; } // В почти ЛЮБОЙ пошаговой системе так или иначе присуствуют очки
    float OverallScore { get; set; } // В основном количество выигранных раундов
    // Актуальный архитектурный вопрос заменить int на float

    bool IsAI { get; }
    // Действие игрока
    // Для ии - начало размышлений над ходом
    // Для человека - возможность взаимодействовать с интерфейсом
    void Act();
    void Init(GlobalFlags gf); // gf lol
    void Init(GlobalFlags gf, TBS_TurnsManager turnsManager);
}
